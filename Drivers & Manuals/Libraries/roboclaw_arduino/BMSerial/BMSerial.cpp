...
uint32_t BMSerial::readdec(uint32_t timeout,bool ignoreleading)
{
	...
	do{
		int16_t c = (int16_t) read(timeout);
		if(c==-1)
			return -1;
...
int16_t BMSerial::peek()
{
	if(_receivePin==0 && _transmitPin==1){
#ifdef USBCON
		return (int16_t) Serial1.peek();
#else
		return (int16_t) Serial.peek();
#endif
	}
#ifdef _BMSERIAL1
	if(_receivePin==_RX1 && _transmitPin==_TX1){
...
#ifdef _BMSERIAL2
	if(_receivePin==_RX2 && _transmitPin==_TX2){
		return (int16_t) Serial2.peek();
	}
#endif
#ifdef _BMSERIAL3
	if(_receivePin==_RX3 && _transmitPin==_TX3){
		return (int16_t) Serial3.peek();
	}
#endif
